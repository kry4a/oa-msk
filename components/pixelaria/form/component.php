...
if( CModule::IncludeModule("iblock") ){
  ...
  if(is_array($arResult["QUESTIONS"])){
    foreach( $arResult["QUESTIONS"] as $FIELD_CODE => $arQuestion ){
      ...
      switch( $arQuestion["FIELD_TYPE"] ){
        case "text":
          $html = '<input type="'.( $arQuestion["CODE"] == "EMAIL" ? "email" : "text" ).'" id="'.$arQuestion["CODE"].'" name="'.$arQuestion["CODE"].'" class="input input--grey '.$required.' '.$phone.'" '.$placeholder.' value="'.$val.'" />';
          break;
        case "integer":
          $html = '<input type="number" id="'.$arQuestion["CODE"].'" name="'.$arQuestion["CODE"].'" class="input input--grey '.$required.'" '.$placeholder.' value="'.$val.'" />';
          break;
        case "date":
          $html = '<input type="date" id="'.$arQuestion["CODE"].'" name="'.$arQuestion["CODE"].'" class="input input--grey date '.$required.'" '.$placeholder.' value="'.$val.'" />';
          break;
        case "html":
          $html = '<textarea id="'.$arQuestion["CODE"].'" rows="3" name="'.$arQuestion["CODE"].'" class="input input--text input--grey '.$required.'" '.$placeholder.'>'.$val.'</textarea>';
          break;
        case "list":
      ...
      $html.='<label class="input-group__label" for="name">'.$arQuestion["CAPTION"].'</label>';
  ...
  if( strlen( $arResult["ERROR"] ) <= 0 ){
    ...
    $arResult["SUBMIT_BUTTON"] = "<button class=\"btn btn--orange btn--full popup__close\" type=\"submit\">".$arParams["SEND_BUTTON_NAME"]."</button><br/><input type=\"hidden\" name=\"form_submit\" value=\"".GetMessage("FORM_ADD")."\">";
  
    $arResult["CLOSE_BUTTON"] = "<div class=\"popup__closer closer\"><b></b><b></b><b></b><b></b></div>";
